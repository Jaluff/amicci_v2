...
class DeliveryController extends Controller
{
    ...
    public function datatable(Request $request)
    {
        ...
        return DataTables::of($query)
            ...
            $deleteForm = "";
            $deleteForm = "";
            if ($row->shipments_count == 0 && $row->status === 'Listo') {
                $deleteForm = "
    ...
    public function destroy(Delivery $delivery)
    {
        ...
        if ($delivery->shipments()->count() > 0 || $delivery->status !== 'Listo') {
            return redirect()->route('deliveries.index')->with('error', 'No se puede eliminar un reparto que tiene guías asignadas o cuyo estado no es "Listo".');
        }
