...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
    <div>
        ...
        <select name="origin_id" id="origin_id"
            ...
            <option value="{{ $branch->id }}" @selected(old('origin_id', $route->origin_id ?? ($defaultOriginId ?? null)) == $branch->id)>
                {{ $branch->name }}
            </option>
            @endforeach
        </select>
...
<div class="mt-8 border-t border-gray-200 dark:border-gray-700 pt-6">
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-bold text-gray-800 dark:text-gray-200 flex items-center gap-2">
            Guías Asignadas
            <span class="assigned-count text-indigo-600 dark:text-indigo-400 bg-indigo-100 dark:bg-indigo-900/40 px-2 py-0.5 rounded-full text-sm">{{ isset($route) ? count($route->shipments ?? []) : 0 }}</span>
        </h3>
